...
const getPopular = async (pageNum) => {
  ...
  let movies = result.results.map((obj) => ({
    id: obj.id,
    title: obj.title,
...
const getMovieDetails = async (id) => {
  ...
  const CastRes = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/credits?api_key=4f07a7cc8af7e84cb0d294a99bf2eacd`
  );
  const castResult = await CastRes.json();
  const cast = castResult.cast.map((obj) => [
    {
      name: obj.name,
...
function* fetchMovieDetails(action) {
  try {
    ...
    yield put({
      type: "MOVIE_DETAILS_FETCH_SUCCEEDED",
